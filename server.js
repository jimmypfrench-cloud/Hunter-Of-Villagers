...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (message.type === "joinRoom") {
      ...
      /*
       * Joining is now allowed no matter what state the room is in —
       * lobby, choosing a level between raids, or mid-raid. It used to
       * be rejected outright once room.state !== "lobby", which made it
       * impossible for a 3rd/4th/5th/6th player to hop in with the room
       * code once the first two had already started playing.
       *
       * The room host (room.players[0]) is unaffected: a brand-new
       * player is always pushed onto the END of room.players, so
       * host status never shifts just because someone joined.
       */

      const hostBeforeJoin = room.players[0] || null;
      ...
      send(ws, {
        ...
        /*
         * Catch-up packet for a player joining a room that's already
         * past the lobby: what level is active (or was last selected),
         * whether it's currently paused, the host's authoritative world
         * state so the new client can render the raid already in
         * progress, and the host's current gear so the newcomer starts
         * mirroring it immediately like every other guest does.
         */
        state: room.state,
        selectedLevel: room.selectedLevel,
        paused: !!room.paused,
        worldSnapshot: room.worldSnapshot,
        allySnapshot: room.allySnapshot,
        hostLoadout: (hostBeforeJoin && hostBeforeJoin !== player)
          ? hostBeforeJoin.loadout
          : null
      });
